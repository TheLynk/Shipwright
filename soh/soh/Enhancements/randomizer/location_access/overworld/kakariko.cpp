...
void RegionTable_Init_Kakariko() {
    ...
    areaTable[RR_KAKARIKO_VILLAGE] = Region("Kakariko Village", SCENE_KAKARIKO_VILLAGE, {
        ...
        LOCATION(RC_KAK_ANJU_AS_CHILD,                          logic->IsChild && logic->AtDay && logic->HasSoul(RG_ANJU_SOUL) && logic->HasItem(RG_CLIMB)), // RANDOTODO only require climb if all 7 cuccos required
        ...
        LOCATION(RC_KAK_SILVER_BOULDER,              logic->IsAdult && logic->CanUse(RG_SILVER_GAUNTLETS) && logic->CanUse(RG_CLIMB)),
